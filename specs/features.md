...
| :-- | :-- | :-- | :-- |
| [15](15-per-voice-mute/) | Per-voice mute | 🚧 | One row of three toggles — **Kick, Snare, Hat**, each behind a hand-drawn pictogram — that silences a groove's voices one at a time, with **Enable all** appearing only once something is muted. Mutes are remembered **per groove**, so switching to bossa does not bring rock's missing hat with it. The change the persona asked for in as many words — *"Fewer beats, fewer instruments, still a groove"* — and the one this table had listed under **Not building**, because that row conflated it with the voices-against-beats grid Sam actually rules out. Built as a **filter over the rendered hit stream**, never a selector that picks a different bar, which is what makes ADR 0010's invariant 3 survive it untouched. `claves` is in no mapping, so the click and the count-in sit outside muting by construction rather than by a branch. Produced [ADR 0015](../docs/adr/0015-thinning-is-a-filter-not-a-selector.md), which binds every future thinning control including the beat toggles still in Candidates. Amended [ADR 0006](../docs/adr/0006-rhythm-instruments-only.md), whose *"muting is per beat and not per voice"* sub-argument this spends — its load-bearing argument, that a bass line chooses the player's key for them, is untouched — and [ADR 0010](../docs/adr/0010-a-marked-bar-modifies-the-figure.md), which now says its invariants bind the written figure rather than the audible result. |
| [16](16-eight-bar-cycle/) | An eight-bar cycle | 2026-09-12 | *"The fills are coming too often."* The marked-bar cycle doubles: ordinary ×3, light, ordinary ×3, fill, so at 100 bpm a fill arrives about every 19 seconds rather than every 10. **One constant and one array**, and all five grooves inherited it at once, because the cycle is machinery rather than data — no groove definition file was touched, which was the spec's own test that the light bar had been left alone. The cycle stopped being positional: `cycle.ts` now names `LIGHT_BAR` and `FILL_BAR`, and `invariants.ts` imports them instead of keeping a second copy of the same numbers in a second file — that duplication is what made *where the checker looks* and *what its message says* able to drift apart. Sam declined the obvious knob: *"Don't read my answer as asking for that control — I didn't."* **No ADR**; the cycle's length was never ADR 0010's subject, which governs what a marked bar may do rather than how often one arrives, though that record's V11 amendment carried a stale `BARS_PER_CYCLE` is 4 and now says so. |
...
| ~~fill-ins for grooves every x bars~~                                                    | — | Shipped as V16: eight-bar cycle, the length a constant rather than a control |
| increase tempo by 5 every time you stop and resume                                     | — |  |
| ~~groove variations (after 2 and 4 bars, still displayed as 1 bar, optional, selectable)~~ | — | Shipped as V8, and **revised by V16**: the 2-and-4 this row asked for proved too frequent in use, so the marked bars moved to 4 and 8 |
| ~~optional count-in 4 clicks for grooves~~                                              | — | Shipped as V9 |
| store settings in localStorage                                                         | — |  |
| ~~disable single instruments from a groove (add icons for the instruments)~~     | — | Shipped as V15, icons and all |
...
| ~~Per-voice mute grid~~ | The **grid** stays ruled out — *"a grid of voices against beats is a drum machine and they did not ask for one."* But V15 shipped per-voice mute as **one row of three toggles**, which is not that, and the persona asked for it in as many words: *"Fewer beats, fewer instruments, still a groove."* This row conflated the two for four changes |
